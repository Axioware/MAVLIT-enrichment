...
Orchestrates all signal enrichment steps in order:
  ...
  3. Shopify detection       — is_shopify / is_woocommerce + social URLs from homepage
  4. Tranco lookup           — in_tranco_list / tranco_rank for brands with domain
  5. Meta Ads                — store raw ad data from Meta Ad Library
  6. YouTube sponsorships    — detect brands being sponsored in YouTube videos
...
def run_signal_enrichment(
    ...
    *,
    ...
    Valid step names:
      wikidata_socials, google_fallback, shopify, tranco, meta_ads, youtube

    If `steps` is None, all steps run.
    Returns a dict mapping step name → rows updated.
    """
    all_steps = [
        "wikidata_socials", "google_fallback", "shopify",
